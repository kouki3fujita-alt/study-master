...
| HFT・執行リスク | [Execution Risk in High-frequency Arbitrage](notes/03_Finance/03_Systematic_Trading/2012_Kozhan_ExecutionRiskHighFrequencyArbitrage.md) | Roman Kozhan, Wing Wah Tham | 2012 | FX三角裁定では競合者が限られた最良気配を奪い合うexecution riskが残り、競争増加がかえって価格差の即時解消を妨げ得ることを示す |
...
| FX option・ヘッジ増幅 | [The Impact of Option Hedging on the Spot Market Volatility](notes/03_Finance/01_Derivatives/2022_Anderegg_OptionHedgingSpotMarketVolatility.md) | Benjamin Anderegg, Florian Ulmann, Didier Sornette | 2022 | DTCCのFX optionポジションから復元したOMM gammaを用い、short gammaのdelta hedgeがspot volatilityを増幅することを検証する |
...
| 指数先物・満期ピニング | [Pinning in the S&P 500 Futures](notes/03_Finance/01_Derivatives/2012_Golez_PinningSP500Futures.md) | Benjamin Golez, Jens Carsten Jackwerth | 2012（2011年SSRN稿） | 先物オプション満期にはS&P 500先物がstrikeへ引かれ、SPX満期前には離れることを示し、charm再ヘッジとITMポジション解消・早期行使の相互作用で説明する |
| オプション注文フロー・価格発見 | [Does Option Trading Convey Stock Price Information?](notes/03_Finance/01_Derivatives/2014_Hu_OptionTradingConveyStockPriceInformation.md) | Jianfeng Hu | 2014（2011年SSRN稿） | オプション誘発の株式注文不均衡と純粋な株式注文不均衡を分解し、前者だけが将来株価を持続的に予測することから、一時的ヘッジ圧力より情報伝達経路を支持する |
...
| ETF option・在庫ヘッジ | [Market Maker or Informed Trader?](notes/03_Finance/01_Derivatives/2025_Chen_MarketMakerInformedTraderOptionReturns.md) | Haiqiang Chen, Zimin Cheng, Yingxing Li, Xiaoqun Liu | 2025 | SSE 50 ETF optionのOIB後にリターンが反転し、情報よりmarket-maker delta hedgeの一時的価格圧力を支持する |
...
| option約定・選択的ヘッジ | [Cross Market Price Discovery and Selective Delta Hedging in the Option Market](notes/03_Finance/01_Derivatives/2025_Dong_CrossMarketPriceDiscoverySelectiveDeltaHedging.md) | Yunjiang Dong | 2025（SSRN稿） | S&P 500個別株でoption約定のspot impactはsingle-leg・LOB取引に集中し、総option出来高で市場影響を判断できないことを示す |
